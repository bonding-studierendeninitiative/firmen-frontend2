...
import { createOrganization, generateOrgInvite, getOrgs, type OrganizationsQueryData } from '@/services';
...
export const load = async ({url, isDataRequest}) => {
	...
	return {
		createForm: superValidate(valibot(CreateOrgRequestSchema)),
		organizationInfo: isDataRequest ? loadOrganizationData(orgsQuery) : await loadOrganizationData(orgsQuery)
	}
