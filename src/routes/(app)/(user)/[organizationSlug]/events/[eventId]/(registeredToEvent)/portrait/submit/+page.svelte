<script lang="ts">
	...
	let superform = superForm<Infer<SubmitPortraitRequest>>(data.submitPortraitForm!!, {
		...
		onResult({ result }) {
			if (result.type === 'error') {
				...
				toast('Failure' + JSON.stringify(result.data), {
					icon: '⚠️'
				});
			}
		},
		onSubmit({ jsonData }) {
			jsonData({
				...$formData,
				eventRegistrationId: data.eventRegistration?.id
			});
		}
	});
	...
	$effect(() => {
		if (data.eventRegistration) {
			$formData.eventRegistrationId = data.eventRegistration.id;
		}
	});
...
<div class="container mx-auto py-8 px-4">
	<div class="flex items-center mb-8">
		<Button variant="ghost" class="mr-4" on:click={() => goto('/portrait-example')}>
	...
	<form action="?/submitPortrait" method="post" use:enhance class="space-y-6">
		...
		<div class="flex justify-between mt-12 max-w-4xl mx-auto">
			...
			{:else}
				<Button type="button" variant="outline" on:click={() => goto('/portrait-example')}>
