	...
	<main class="container mx-auto py-12 px-4 md:px-6">
		<header>
			<h1 class="text-white text-3xl font-extrabold">
				{$_('user-pages.organizations.organizations')}
			</h1>
			<h4 class="text-white max-w-prose text-balance">
				{$_('user-pages.organizations.organizationsSubHeading')}
			</h4>
		</header>
		<section class="my-10 flex flex-col gap-4">
			<h2 class="text-white text-lg font-extrabold">
				{$_('user-pages.organizations.myOrganizations')}
			</h2>
			{#if data.memberships}
				{#if data.memberships.length >= 0}
					<NoDataFound
						heading={$_('user-pages.organizations.noOrgMembershipsFound')}
						subHeading={$_('user-pages.organizations.noOrgMembershipsFoundDescription')}
						buttonText={$_('user-pages.organizations.newOrganization')}
						onButtonClick={() => {
							isOpen = true;
						}}
					/>
				...
				<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
					{#each data.memberships as { organizationName, organizationSlug, membershipType }, index (index)}
						<div class="bg-white p-6 rounded-lg shadow-md">
							<div class="mb-4 flex flex-col gap-2 justify-items-start">
								...
								<span
									class="w-fit rounded text-xs font-mono tracking-wide px-2 py-1 bg-gray-100 text-muted-foreground"
									>{organizationSlug}</span
								>
							</div>
							<div class="flex items-center justify-between">
								<span class=" text-sm text-muted-foreground">{membershipType}</span>
								<Button onClick={() => {}} classes="outline">Leave</Button>
							</div>
						</div>
					{/each}
				</div>
