...
export const updateMetadata = command(updateUserDetailsSchema, async (input) => {
  try {
    ...
    await ctx.auth.updateUser({ body: { metadata: JSON.stringify({ public: { phone: input.phone, title: input.title, position: input.position } }) }, headers: { Authorization: `Bearer ${ctx.session.token}` } });
    return { ok: true };
  } catch (error) {
