...
import { object, string, nullish, number, safeParseAsync } from 'valibot';
...
export const createBuyOption = form(async (formData) => {
	const ctx = await createAdminContext();
	...
	if (result.success) {
		const response = await ctx.adminApi.post('/api/v2/admin/event/{eventId}/buy-option', {
			path: { eventId: result.output.eventId },
			body: result.output
		});
		await getBuyOptions({ eventId: result.output.eventId }).refresh();
...
// update buy option
export const updateBuyOption = form(async (formData) => {
	const ctx = await createAdminContext();
	const result = await safeParseAsync(
		UpdateBuyOptionRequestSchema,
		Object.fromEntries(formData.entries())
	);
	if (result.success) {
		const response = await ctx.adminApi.put(
			'/api/v2/admin/event/{eventId}/buy-option/{buyOptionId}',
			{
				path: { eventId: result.output.eventId, buyOptionId: result.output.buyOptionId },
				body: result.output
			}
		);
		await getBuyOptions({ eventId: result.output.eventId }).refresh();
