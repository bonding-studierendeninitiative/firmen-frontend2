import { GetBillingAddressTemplateForOrganizationResponse, BillingAddressTemplate, CreateBillingAddressTemplateForm } from "@schema";
...
export const billingAddressTemplateRouter = router({
    getAll: authorizedOrgMemberProcedure.input((input) => parse(
        object({
        ...
        .query(async ({ ctx, input }) => {
            const response = await ctx.api.request("get", "/api/v2/organization/{organizationId}/billing-address-template", {
                path: {
    ...
    create: authorizedOrgMemberProcedure
        ...
        .mutation(async ({ ctx, input }) => {
            const response = await ctx.api.request("post", "/api/v2/organization/{organizationId}/billing-address-template", {
                path: {
    ...
    delete: authorizedOrgMemberProcedure
        ...
        .mutation(async ({ ctx, input: billingAddressTemplateId }) => {
            const response = await ctx.api.request("delete", "/api/v2/organization/{organizationId}/billing-address-template/{billingAddressTemplateId}", {
                path: {
                    organizationId: ctx.session.orgId,
